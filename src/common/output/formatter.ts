...
function severityColor(severity: number): keyof typeof COLORS {
  switch (severity) {
    case 5:
      return 'red';
    case 4:
...
export function printBanner(scanType: 'container' | 'code', target: string): void {
  const width = 70;
...
export function printSummaryTable(summary: VulnerabilitySummary): void {
  const width = 50;
  const line = '─'.repeat(width);
  ...
  const rows = [
    { label: 'Critical', value: summary.critical, color: 'red' as const },
    { label: 'High', value: summary.high, color: 'magenta' as const },
    { label: 'Medium', value: summary.medium, color: 'yellow' as const },
    { label: 'Low', value: summary.low, color: 'cyan' as const },
    { label: 'Informational', value: summary.informational, color: 'white' as const },
  ];
  ...
  console.log(
    colorize(`│`, 'white') +
      ` ${'TOTAL'.padEnd(15)}${colorize(summary.total.toString().padStart(5), 'bold')}`.padEnd(
        width + 9
      ) +
      colorize(`│`, 'white')
  );
...
export function printTopVulnerabilities(report: SarifReport, limit: number = 10): void {
  ...
  for (const run of report.runs || []) {
    ...
    for (const result of run.results || []) {
      ...
      findings.push({
        ...
        package: (result.properties?.packageName as string) || '-',
        cves: (result.properties?.cves as string[]) || [],
      });
    }
  }
  ...
  console.log(
    `${'SEV'.padEnd(10)}${'PACKAGE'.padEnd(25)}${'CVE'.padEnd(20)}${'TITLE'.padEnd(25)}`
  );
  console.log('─'.repeat(80));
...
export function printThresholdResult(
...
export function printPolicyResult(result: 'ALLOW' | 'DENY' | 'AUDIT' | 'NONE'): void {
...
export function printFinalStatus(passed: boolean, reasons: string[] = []): void {
  const width = 50;
  ...
  if (passed) {
    ...
    if (reasons.length > 0) {
      ...
      for (const reason of reasons) {
        console.log(
          colorize('║', 'red') + `  • ${reason.substring(0, 46)}`.padEnd(width) + colorize('║', 'red')
        );
      }
    }
...
export function printReportLocations(locations: { type: string; path: string }[]): void {
