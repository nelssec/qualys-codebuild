...
async function main() {
    ...
    (0, formatter_1.printBanner)('container', config.imageId);
    ...
    if (config.sendToSecurityHub && sarifReport) {
        ...
        if (securityHubOptions) {
            await (0, securityhub_1.importFindingsToSecurityHub)(sarifReport, config.imageId, 'container', securityHubOptions);
        }
    }
    ...
    if (config.sendEventBridgeNotification) {
        const eventOptions = (0, eventbridge_1.buildEventFromEnvironment)('container', config.imageId, scanPassed ? 'PASSED' : 'FAILED', result.policyResult, summary, reportLocations.find((r) => r.type === 'S3')?.path);
