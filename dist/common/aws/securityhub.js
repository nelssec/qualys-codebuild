...
exports.importFindingsToSecurityHub = importFindingsToSecurityHub;
exports.buildSecurityHubOptionsFromEnv = buildSecurityHubOptionsFromEnv;
const client_securityhub_1 = require("@aws-sdk/client-securityhub");
let client = null;
function getClient() {
    if (!client) {
        client = new client_securityhub_1.SecurityHubClient({});
    }
    return client;
}
function mapSeverityToSecurityHub(qualysSeverity) {
...
async function importFindingsToSecurityHub(sarifReport, target, scanType, options) {
    const securityHubClient = getClient();
    const findings = [];
    const productArn = options.productArn ||
    ...
    for (const run of sarifReport.runs || []) {
        ...
        for (const result of run.results || []) {
            ...
            const qid = result.properties?.qid;
            const cves = result.properties?.cves;
            const packageName = result.properties?.packageName;
            const installedVersion = result.properties?.installedVersion;
    ...
    for (let i = 0; i < findings.length; i += batchSize) {
        ...
        const command = new client_securityhub_1.BatchImportFindingsCommand({
            Findings: batch,
        });
...
function buildSecurityHubOptionsFromEnv() {
